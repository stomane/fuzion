...
namespace Fuzion.Icons
{
    public static class IconTools
    {
        ...
        public static string SaveJumboIcon(string iconPath, int index)
        {
            string ext = System.IO.Path.GetExtension(iconPath);


            if(ext == ".exe")
            {
                IntPtr hIcon = GetJumboIcon(GetIconIndex(iconPath)); //"*." + ext)); //previous code

                // from native to managed
                using (Icon ico = (Icon)Icon.FromHandle(hIcon).Clone())
                {
                    // save to file (or show in a picture box)
                    ico.ToBitmap().Save(Fuzion.MainWindow.DefaultAssetPath + @"Icons\" + index + ".png", ImageFormat.Png);
                }
                _ = Shell32.DestroyIcon(hIcon); // don't forget to cleanup
            } else
            {
                IntPtr hIcon = GetJumboIcon(GetIconIndex(iconPath)); //"*." + ext)); //previous code
            ...
            return Fuzion.MainWindow.DefaultAssetPath + @"Icons\" + index + ".png";
        }
        ...
        public static IntPtr GetJumboIcon(int iImage)
        {
            ...
            _ = Shell32.SHGetImageList(Shell32.SHIL_JUMBO, ref guil, ref spiml);

            IntPtr hIcon = IntPtr.Zero;
            _ = spiml.GetIcon(iImage, Shell32.ILD_TRANSPARENT | Shell32.ILD_IMAGE, ref hIcon);
